...
const Investeedashboardinvestmentdetail = () => {
  const { id } = useParams();
  ...
  function getInvestment() {
    ...
    fetch(`${process.env.REACT_APP_FETCH_URL_}/api/investee/get-investment-detail`, {
      method: "GET",
      headers: {
        "id": id,
        'token': token,
  ...
  useEffect(() => {
    if (window.localStorage.getItem('token')) {
      document.title = "Investify | Investee-Investment-Detail";
      getInvestment();
    } else {
      navigate("/user-login");
    }
...
export default Investeedashboardinvestmentdetail